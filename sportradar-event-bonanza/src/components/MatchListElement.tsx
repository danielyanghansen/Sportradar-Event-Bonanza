...
import { getMatchScore, getMatchScores, getMatchName } from '../utils/matchUtils';
  ...
  return (
    ...
    >
      ...
      <div style={{display: "flex", justifyContent: "space-between"}}>
        <p style={{ width:"50%", textAlign: "left", fontWeight: "600" }}>{getMatchName(match)[0]}</p>
        <p>vs</p>
        <p style={{ width:"50%", textAlign: "right", fontWeight: "600" }}>{getMatchName(match)[1]}</p>
      </div>
      <p style={{ fontSize: '2.5em' }}>
        <strong>{getMatchScores(match)[0]}</strong> - <strong>{getMatchScores(match)[1]}</strong>
      </p>
