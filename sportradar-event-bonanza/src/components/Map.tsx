...
const Map = () => {
  ...
  const createToolTipOnClick = () => {
    ...
    const addPointsToMap = (lat: number, lng: number, icon?: string, zoom?: number) => {
      map.current && map.current.on("load", () => {
        ...
        // sleep 3 secs
        setTimeout(() => {
        map.current?.flyTo({ center: [lat, lng], zoom: zoom || 3, duration: 5000 });
        }, 3000);
      });
    }
